...
namespace Anubis
{
  public class GcmBlockCipher : IAeadBlockCipher
  {
    ...
    public virtual int GetOutputSize(int len) => this.forEncryption ? len + this.bufOff + this.macSize : len + this.bufOff - this.macSize;

    public virtual int GetUpdateOutputSize(int len) => (len + this.bufOff) / 16 * 16;

    public virtual int ProcessByte(byte input, byte[] output, int outOff) => this.Process(input, output, outOff);
